...
public class CouponService {
  ...
  /**
   * 1. Race Condition 발생 가능성 findByIdWithLock으로 쿠폰 정책에 대해 락을 걸지만, countByCouponPolicyId와 실제 쿠폰 저장 사이에 갭이 존재 여러 트랜잭션이
   * 동시에 카운트를 조회하고 조건을 통과한 후 쿠폰을 저장할 수 있음 결과적으로 totalQuantity보다 더 많은 쿠폰이 발급될 수 있음
   * <p>
   * 2. 성능 이슈 매 요청마다 발급된 쿠폰 수를 카운트하는 쿼리 실행 쿠폰 수가 많아질수록 카운트 쿼리의 성능이 저하될 수 있음 PESSIMISTIC_LOCK으로 인한 병목 현상 발생 가능
   * <p>
